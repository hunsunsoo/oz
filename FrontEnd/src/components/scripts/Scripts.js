...
const Scripts = () => {
  ...
  const IntrodialogueData = [
    // 인트로 일러스트
    {
      ...
      character: "",
      message:
        "산책 중이던 오즈는 갑작스러운 토네이도에,도로시가 오즈의 나라로 날아가게 되는데...",
    },
    {
      index: 2,
      character: "",
  ...
  const handleNextClick = () => {
    // Increment the currentDialogueIndex to display the next message
    setCurrentDialogueIndex((prevIndex) => prevIndex + 1);
  };

  // Check if the currentDialogueIndex exceeds the length of dialogueData
  const isEndOfDialogue = currentDialogueIndex >= dialogueData.length;
  ...
  return (
    <div>
      {isEndOfDialogue ? (
        ...
        <div>
          <div>
            {dialogueData[currentDialogueIndex].character}
            <br />
            {dialogueData[currentDialogueIndex].message}
          </div>
