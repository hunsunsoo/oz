...
const GameComp = (props) => {
  ...
  const subscribeToStage1Start = () => {
    ...
    const subscription = client.subscribe(
      `/sub/socket/calculation/start/${roundId}/${sessionId}`,
      (message) => {
        console.log("Received message:", message.body);
  ...
  const subscribeToStage1SelectCells = () => {
    ...
    client.subscribe(
      `/sub/socket/calculation/helpersubmit/${roundId}/${sessionId}`,
      (message) => {
        console.log("Received message:", message.body);
  ...
  const sendStage1SelectCells = () => {
    ...
    try {
      ...
      client.send(
        `/pub/calculation/helpersubmit/${roundId}`,
        {},
        JSON.stringify(message)
      );
  ...
  const subscribeToStage1GetAnswer = () => {
    ...
    client.subscribe(
      `/sub/socket/calculation/getanswer/${roundId}/${sessionId}`,
      (message) => {
        console.log("Received message:", message.body);
        console.log("문제가 뭘까요?");
  ...
  const subscribeToStage1SelectAns = () => {
    ...
    client.subscribe(
      `/sub/socket/calculation/submitanswer/${roundId}/${sessionId}`,
      (message) => {
        console.log("Received message:", message.body);
  ...
  const handleMarksClick = (MarksValue) => {
    ...
    if (isMarksSelected) {
      // 이미 선택된 칸이라면 해당 값을 배열에서 제거
      const updatedMarks = selectedMarks.filter(
        (value) => value !== MarksValue
      );
    ...
    return (
      <div className={style.compStyle}>
        <div className={style.background_G1}>
          ...
          <div className={style.ansSubmitBtn} onClick={sendStage1SelectAns}>
            정답제출
          </div>
      ...
      <div className={style.compStyle}>
        <div className={style.background_G1}>
          ...
          <div
            ...
            style={{
              display: isStartBtnActive && host === 1 ? "flex" : "none",
            }}
            onClick={sendStage1Start}
          >
            게임 시작
          </div>
          <img
