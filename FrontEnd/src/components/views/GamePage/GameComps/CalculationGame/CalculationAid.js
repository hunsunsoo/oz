...
import { NumberBoard, AlphaBoard, MathBoard, AnsBoard } from "./CalBoard";
import style from "./Calculation.module.css";
import GameModal from "../GameModal/GameModal";

const CalculationAid = ({
  boardData,
  myRole,
  client,
  sessionId,
    ...
    const subscribeToCellSelect = () => {
      const trySubscribe = () => {
        ...
        const subscription = client.subscribe(
          `/sub/socket/calselect/${sessionId}`,
          (message) => {
  ...
  const CalculationAidCellSelectPublisher = async (
    role,
    pubState,
    cellValue,
    loc
