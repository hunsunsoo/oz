...
const DrawingGame = ({client, sessionId, myRole, handleindexSet}) => {
    ...
    useEffect(() => {
        ...
        const trySubscribe = () => {
            ...
            const subscription = client.subscribe(`/sub/socket/draw/pass/${sessionId}`, (message) => {
              console.log('Received message:', message.body);
    ...
    useEffect(() => {
        ...
        const trySubscribe = () => {
            ...
            const subscription = client.subscribe(`/sub/socket/draw/data/${sessionId}`, (message) => {
              console.log('Received message:', message.body);
              
              try {
                const resJson = JSON.parse(message.body);
                if(resJson.data === -1){
                    alert("정답을 틀려 게임이 다시 시작됩니다. 준비 화면으로 돌아갑니다.");
    ...
    switch (myRole) {
        case 1:
            DrawingGameRenderingState = <DrawingDorothy client={client} sessionId={sessionId} myRole={myRole} myUserId={myUserId} currentRole={currentRole} />
        break;
        case 2:
        case 3:
        case 4:
            DrawingGameRenderingState = <DrawingAid client={client} sessionId={sessionId} keyword={keyword} myRole={myRole} myUserId={myUserId} currentRole={currentRole}/>
        break;
    }

    return (
        <div className={style.drawingContainer}>
          {isStart ? DrawingGameRenderingState : <DrawingReady onHandleStart={drawingGameStartPublisher}/>}
        </div>
      );
}
